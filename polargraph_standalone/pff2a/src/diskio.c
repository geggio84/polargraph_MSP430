...
#define SELECT()	P1OUT &= ~(0x01)	/* CS = L */
#define	DESELECT()	P1OUT |= (0x01)	/* CS = H */
#define	MMC_SEL		!(P1OUT & (0x01))	/* CS status (true:CS == L) */
	...
	if (buff) {		/* Send data bytes */
		...
		if (sa) {	/* Initiate sector write process */
			...
			if ((spi_receive() & 0x1F) == 0x05) {	/* Receive data resp and wait for end of write process in timeout of 500ms */
				for (bc = 5000; spi_receive() != 0xFF && bc; bc--) __delay_cycles(1600); //dly_100us();	/* Wait ready */
				if (bc) res = RES_OK;
			}
			DESELECT();
			spi_receive();
